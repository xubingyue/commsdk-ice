...
void ServerI::setConnectionInfoCallback(ServerConnectionInfoCallback connectionInfoCallback)
{
...
void ServerI::addClient(const Ice::Identity& id, const Ice::Current& curr)
{
	...
	UVSS::ClientPrx clientProxy = UVSS::ClientPrx::uncheckedCast(curr.con->createProxy(id));
	...
	if (this->connectionInfoCallback != 0) {
		this->connectionInfoCallback(0, std::string("客户端 " + endpoint.str() + ": 已连接").c_str());
	}
}
			...
			for (std::map<UVSS::ClientPrx, std::string>::const_iterator it = clientProxyToEndpoint.begin(); it != clientProxyToEndpoint.end(); ++it) {
				try {
					...
					if (!this->isDestroyed) {
						...
						if (this->connectionInfoCallback != 0) {
							this->connectionInfoCallback(-1, std::string("客户端 " + endpoint + ": 已断开").c_str());
						}
		...
		const std::string& extension)
{
	...
	for (std::map<UVSS::ClientPrx, std::string>::const_iterator it = this->clientProxyToEndpoint.begin(); it != this->clientProxyToEndpoint.end();) {
		try {
